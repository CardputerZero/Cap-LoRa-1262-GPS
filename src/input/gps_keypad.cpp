...
{
    static const bool enabled = envEnabled("CAP_GPS_KEYBOARD_DEBUG", false);
    return enabled;
}
...
{
#if !LV_USE_SDL && defined(__linux__)
    if (const char* device_path = std::getenv("CAP_GPS_KEYBOARD_DEVICE")) {
...
bool GpsKeypad::openDevice(const std::string& path, bool require_app_keys)
{
    ...
    const bool grab_input = envEnabled("CAP_GPS_KEYBOARD_GRAB", false);
